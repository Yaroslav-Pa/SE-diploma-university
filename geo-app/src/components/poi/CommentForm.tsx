      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <label className="text-xs text-gray-500 uppercase font-bold tracking-wide">Attach Photos (optional)</label>
            {photos.length > 0 && (
              <button type="button" onClick={() => setPhotos([])} className="text-[10px] text-red-500 font-bold hover:underline">
                Clear All
              </button>
            )}
          </div>
            ...
            <div className="flex gap-2 mt-2 overflow-x-auto pb-2">
              {photos.map((file, idx) => (
                <div key={idx} className="relative shrink-0 w-12 h-12 group">
                  <img src={URL.createObjectURL(file)} alt="preview" className="w-full h-full object-cover rounded border border-gray-200 dark:border-slate-700" />
                  <button 
                    type="button"
